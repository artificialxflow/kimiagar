      ...
      <div className="space-y-4">
        {displayedTransactions.length > 0 ? (
          displayedTransactions.map((transaction: any, index: number) => {
            ...
            return (
              <div key={transaction.id || index} className="flex items-center justify-between p-4 bg-slate-50 rounded-lg hover:bg-slate-100 transition-colors">
      ...
      {isExpanded && transactions.length > 3 && (
        <div className="space-y-4 mt-4 pt-4 border-t border-slate-200">
          {transactions.slice(3).map((transaction: any, index: number) => {
            ...
            return (
              <div key={transaction.id || index + 3} className="flex items-center justify-between p-4 bg-slate-50 rounded-lg hover:bg-slate-100 transition-colors">
                <div className="flex items-center">
                  <div className={`w-10 h-10 rounded-full flex items-center justify-center mr-3 ${
                    transaction.type === 'DEPOSIT' ? 'bg-green-100' : 'bg-red-100'
                  }`}>
